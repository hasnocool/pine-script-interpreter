...
- **Produced completed trades:** 2,021
- **Ran without completed orders:** 3,781
- **Could not be evaluated:** 279
- **Saved partial interrupted results:** 137
- **Used an explicit approximation:** 3,842
- **Used a local Pine library:** 74
...
| --- | ---: | --- |
| Backtested with trades | 2,021 | The runtime ran and closed at least one trade. |
| No orders | 3,781 | The runtime ran but did not close a trade on this data. |
| Validation errors | 142 | The current runtime could not evaluate part of the strategy. |
| Execution-limit stops | 137 | The strategy exceeded the safety step budget. |
...
- **582** strategies with trades finished positive; **924** finished negative; **515** were approximately flat.
- The median return among trade-producing strategies was **0.00%**; the average was **1202629.89%**.
...
| --- | ---: | ---: |
| Runaway loop or excessive runtime steps | 137 | 49.10% |
| unknown Pine builtin | 38 | 13.62% |
| imported Pine library was not found | 33 | 11.83% |
| Unknown Pine identifier or constant | 29 | 10.39% |
| Required numeric value was unavailable | 12 | 4.30% |
| unknown strategy member | 4 | 1.43% |
| circular Pine library import | 2 | 0.72% |
| long stop must be below limit | 2 | 0.72% |
| Could not find relevant COT data based on the current symbol and the source sear | 2 | 0.72% |
| Unsupported call target | 2 | 0.72% |
| Chart timeframe must be BELOW the signal timeframe. Recommended | 1 | 0.36% |
| The size of the real (array x) and imaginary (array y) parts of the input does n | 1 | 0.36% |
...
| ICT Entry V2 [TS Indie]__wm9e7Wca | execution_limit | 473 | 0 | 100,000.00 | flat |
| ICT Entry V1 [TS Indie]__mVdZBLJ7 | execution_limit | 470 | 0 | 100,000.00 | flat |
...
| --- | --- | ---: | ---: | ---: | ---: |
| TURKS Tiered Unit Risk Kernel Strategy__woRXiMlC | execution_limit | 15.316s | 250001 | 62 | 0 |
